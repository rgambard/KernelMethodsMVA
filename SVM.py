...
step_size, lambd, n_steps = 1e-6, 1e-4,10000
print(step_size,lambd,n_steps)
K = euc_kernel(x_train,x_train)
alpha,support_vectors,mu,losses,non_sup = SVM_solver(x_train,y_train,K,lambd,step_size,n_steps)
plt.plot(losses)
#plt.show()

prediction_train = predict(alpha,support_vectors,euc_kernel,x_train)
print(np.mean(prediction_train*y_train>0))

prediction_val = predict(alpha,support_vectors,euc_kernel,x_val)
